...
class spbcta_Plugin
{
	...
	<div class="spbctawrap">
		<div class="spbcta_tip"><span>Tip:</span> Copy & Paste shortcodes in your post/page to show the button.</div>
		
		<h2 class="spbcta_backend_headline">Coupon Reveal Button</h2>
		<?php
		if(isset($_GET['func']) && $_GET['func']=='add_cta'){
			// 'ADD TABLE' UI
			$this->spbcta_setup_UI('','','','','','','','');
		} else if(isset($_GET['func']) && isset($_GET['editnum']) && $_GET['func']=='edit_cta' && isset($_GET['_wpnonce']) && wp_verify_nonce($_GET['_wpnonce'], 'edit_cta')) {
			// 'EDIT TABLE' UI
			...
			if($table){
				...
				// 'LIST TABLES' UI
			printf('<a class="spbcta_btn spbcta_btn_new_table" href="%s">%s</a>', admin_url('admin.php?page='.$this->page_slug."&func=add_cta"), 'Add New Button');
			if(isset($_GET['deletedCTA'])){
				echo '<p class="spbcta_removed">The button "'.esc_attr($_GET['deletedCTA']).'" has successfully been deleted.</p>';
			}
			echo "<table class='spbcta_backend-table'> 
			<tr>
			<th>Button Name</th>
			...
			if($all){
				...
				for($i=0;$i<$tableCount;$i++){
					printf('<tr>
						...
						, $all[$i]['name'], $all[$i]['id'], wp_nonce_url(admin_url('admin.php?page='.$this->page_slug."&func=edit_cta".'&editnum='.$all[$i]['id']),'edit_cta'),'Edit Button'
						, wp_nonce_url(admin_url('admin.php?page='.$this->page_slug."&func=copy_CTA&CTAName=".$all[$i]['name'].'&copyNum='.$all[$i]['id']), 'copy_CTA'),'Copy Button'
						, wp_nonce_url(admin_url('admin.php?page='.$this->page_slug."&func=delete_CTA&CTAName=".$all[$i]['name'].'&deleteNum='.$all[$i]['id']), 'delete_CTA'),'Delete Button');
...
private function spbcta_setup_UI($CTAName, $CTAColor, $CTAutext, $CTAureveal, $CTAlink, $CTABlank, $CTAnofollow,$CTAStyle){
	...
	<form method="post" name="saveCTA">
		...
		<div class="spbcta_tableshortcode">
			...
			<div class="spbcta-preview-wrapper-center">
				...
				<div class="spbcta-preview-wrapper"> 
					...
					<div class="reveal__button__wrapper reveal__button__<?php echo esc_attr($CTAStyle); ?>__design">
						<a href="#spbcta_btn_preview" class="reveal__button__link" onclick="spbctaNM.func.spbcta_pass(<?php echo "'".base64_encode(esc_attr($CTAureveal))."'"?>,this);">
							<span class="reveal__button__text" style="background:<?php echo esc_attr($CTAColor[0]); ?>; color:<?php echo esc_attr($CTAColor[1]); ?>;"><?php echo esc_attr($CTAutext); ?></span>
							<span class="reveal__button__hidden__content" style="color:<?php echo esc_attr($CTAColor[3]); ?>;border:2px dashed <?php echo esc_attr($CTAColor[4]); ?>;background-color:<?php echo esc_attr($CTAColor[2]); ?>;"><?php echo mb_substr(esc_attr($CTAureveal), -3) ?></span>
						</a>
					</div>
...
private function spbcta_validateSanitize($input, $type) {
	...
	if($type == 'hex'){
		...
		} else { $return = ['#00c6a5','#ffffff','#e8e8e8','#6f6f6f','#00c6a5'];}
	}
	if($type == 'bit'){
		if(strlen($input) > 1){
			$return = 0;
...
function spbcta_handleSC( $atts ){
	...
	if($table){
		...
		$CTAlink = $table['link'];
		if(!$CTAlink){$CTAlink = '#spbcta_ph'.$CTAId;}
		...
		<div class="reveal__button__wrapper reveal__button__<?php echo esc_attr($CTAStyle); ?>__design">
			...
			<a id="spbcta<?php echo $CTAId?>" href="<?php echo esc_url($CTAlink); ?>" <?php if($CTABlank == 1){?>target="_blank"<?php } ?> class="reveal__button__link" onclick="spbctaNM.func.spbcta_pass(<?php echo "'".base64_encode(esc_attr($CTAureveal))."'"?>,this,'<?php echo esc_url($CTAlink); ?>',<?php echo $CTABlank ?>);">
				<span class="reveal__button__text" style="background:<?php echo $CTAColor[0]?>; color:<?php echo $CTAColor[1]?>;"><?php echo esc_attr($CTAutext); ?></span>
				<?php if($CTAureveal){ ?><span class="reveal__button__hidden__content" style="color:<?php echo $CTAColor[3]?>;border:dotted 2px <?php echo $CTAColor[4]?>;background-color:<?php echo $CTAColor[2]?>;"><?php echo mb_substr(esc_attr($CTAureveal), -3) ?></span><?php } ?>
			</a>
		</div>
		<?php
		return ob_get_clean();
